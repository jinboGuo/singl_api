...
from new_api_cases.qa_prepare_datas import create_datasetjob_data,\
    create_datacoljob_data,create_rule_import,get_success_exehistoryjobid

cases_dir = quality_cases_dir
case_table = load_workbook(cases_dir)
qa_master=quality_sheet
    ...
    for i in range(2, all_rows+1):
        ...
        if request_method_upper:
            if is_run =='Y' or is_run=='y':
                ...
                elif isinstance(request_data, list) and len(request_data)>1:
                    if request_method_upper == 'POST':
                        post_request_result_check(row=i, column=8, url=request_url.format(request_data[1]), headers=get_headers(), data=request_data[0], table_sheet_name=case_table_sheet)
                    elif request_method_upper == 'GET':
                        get_request_result_check(url=request_url.format(request_data[1]), headers=get_headers(), data=request_data[0], table_sheet_name=case_table_sheet, row=i, column=8)
                    elif request_method_upper == 'PUT':
                        put_request_result_check(url=request_url.format(request_data[1]), row=i, data=request_data[0], table_sheet_name=case_table_sheet, column=8, headers=get_headers())
                    elif request_method_upper == 'DELETE':
                        delete_request_result_check(url=request_url.format(request_data[1]), data=request_data[0], table_sheet_name=case_table_sheet, row=i, column=8, headers=get_headers())
                    else:
                        log.info("请求方法%s不在处理范围内" % request_method)
                elif  len(request_data)==1 and isinstance(request_data, str):
                    """根据不同的请求方法，进行分发"""
                    if request_method_upper == 'POST':
                        ...
                        delete_request_result_check(url=request_url, param=request_data, table_sheet_name=case_table_sheet, row=i, column=8, headers=get_headers())
                    else:
                        log.info("请求方法%s不在处理范围内" % request_method)
                else:
                    """根据不同的请求方法，进行分发"""
...
def post_request_result_check(row, column, url, headers, data, table_sheet_name):
    ...
    try:
        ...
        if  case_detail == '质量数据集任务-新建':
            new_data = create_datasetjob_data(data)
            response = requests.post(url=url, headers=headers, data=json.dumps(new_data))
            log.info("response data：%s %s" % (response.status_code, response.text))
            clean_vaule(table_sheet_name, row, column)
            write_result(sheet=table_sheet_name, row=row, column=column, value=response.status_code)
            write_result(sheet=table_sheet_name, row=row, column=column + 4, value=response.text)
        elif  case_detail == '质量字段任务-新建':
            new_data = create_datacoljob_data(data)
            response = requests.post(url=url, headers=headers, data=json.dumps(new_data))
            log.info("response data：%s %s" % (response.status_code, response.text))
            clean_vaule(table_sheet_name, row, column)
            write_result(sheet=table_sheet_name, row=row, column=column, value=response.status_code)
            ...
            write_result(sheet=table_sheet_name, row=row, column=column, value=response.status_code)
            write_result(sheet=table_sheet_name, row=row, column=column + 4, value=response.text)
        elif case_detail=='异常数据预览':
            sta=get_success_exehistoryjobid({'成功':"数据集",'exe':'all'})
            response = requests.post(url=url.format(sta['mappingId']), headers=headers, data=json.dumps(sta))
            log.info("response data：%s %s" % (response.status_code, response.text))
            clean_vaule(table_sheet_name, row, column)
            write_result(sheet=table_sheet_name, row=row, column=column, value=response.status_code)
            write_result(sheet=table_sheet_name, row=row, column=column + 4, value=response.text)
...
def read_data():
        ...
        table = data.sheet_by_name(qa_master)
        """获取总行数"""
        nrows = table.nrows
        if nrows > 1:
            """获取第一行的内容，列表格式"""
...
class CheckResult(unittest.TestCase):
    ...
    def assert_deal(self, key_word, relation, expect_text, response_text, response_text_dict, row, column):
        ...
        """
        if  expect_text==None or expect_text=='' or response_text_dict==None or response_text_dict=='':
