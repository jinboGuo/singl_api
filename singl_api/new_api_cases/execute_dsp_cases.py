...
def deal_request_method():
    for i in range(2, all_rows+1):
        ...
        Logger().get_log().info("request  data：%s" %request_data)
        ...
        if request_method:
            ...
            if api_name == 'admin':  # 用例需要使用admin用户登录后操作
                ...
                if request_method_upper == 'POST':
                    # 调用post方法发送请求
                    post_request_result_check(row=i, column=8, url=request_url, host=host, headers=get_headers_admin(host),
                                              data=request_data, table_sheet_name=case_table_sheet)

                elif request_method_upper == 'GET':
                    # 调用GET请求
                    get_request_result_check(url=request_url, host=host, headers=get_headers_admin(host), data= request_data,
                    ...
                    Logger().get_log().info("请求方法%s不在处理范围内" %request_method)
            else:
                # 消费者登录根据不同的请求方法，进行分发
                if request_method_upper == 'POST':
                    # 调用post方法发送请求
                    post_request_result_check(row=i, host=host, column=8, url=request_url, headers=get_headers_customer(host),
                                                    data=request_data, table_sheet_name=case_table_sheet)

                elif request_method_upper == 'GET':
                    # 调用GET请求
                    get_request_result_check(url=request_url, host=host, headers=get_headers_customer(host), data=request_data,
                                             table_sheet_name=case_table_sheet, row=i, column=8)

                elif request_method_upper == 'PUT':
                    put_request_result_check(url=request_url, host=host, row=i, data=request_data, table_sheet_name = case_table_sheet, column=8, headers=get_headers_customer(host))

                elif request_method_upper == 'DELETE':
                    delete_request_result_check(url=request_url, host=host, data=request_data,table_sheet_name= case_table_sheet,row=i,column=8, headers=get_headers_customer(host))

                else:
                    Logger().get_log().info("请求方法%s不在处理范围内" %request_method)
        else:
            Logger().get_log().info("第 %d 行请求方法为空" %i)
    #  执行结束后保存表格
...
def post_request_result_check(row, column, url, host, headers, data, table_sheet_name):
    try:
        ...
        if '数据资源关联' in case_detail:
            Logger().get_log().info("开始执行：%s" %case_detail)
            Logger().get_log().info("request   url：%s" %url)
            new_data = resource_data_save(data)
            new_data = json.dumps(new_data, separators=(',', ':'))
            response = requests.post(url=url, headers=headers, data=new_data)
            Logger().get_log().info("response data：%s %s" %(response.status_code, response.text))
            ...
            Logger().get_log().info("开始执行：%s" %case_detail)
            Logger().get_log().info("request   data：%s" %data)
            response = requests.post(url=url, headers=headers, json=data)
            Logger().get_log().info("response data：%s %s" %(response.status_code, response.text))
            clean_vaule(table_sheet_name, row, column)
            write_result(sheet=table_sheet_name, row=row, column=column, value=response.status_code)
            ...
            Logger().get_log().info("request   url：%s" %new_url)
            new_data = json.dumps(new_data, separators=(',', ':'))
            ...
            new_data = json.dumps(new_data, separators=(',', ':'))
            Logger().get_log().info("request   data：%s" %new_data)
        ...
        if data:
            if '删除申请记录' in case_detail:
                ...
                while '"executedTimes":0'in response.text:
                    Logger().get_log().info("再次查询前：%s" %(response.status_code, response.text))
                    response = requests.get(url=new_url, headers=headers)
                    time.sleep(5)
                    count_num += 1
                    if count_num == 80:
                ...
                if '&' in str(data):  # 包含多个参数并以&分割
                    ...
                    for i in range(len(parameters)):
                        if parameters[i].startswith('select id from'):
                            try:
                                ...
                                Logger().get_log().info("第%s行参数没有返回结果" %row)
                    ...
                    if len(parameters) == 1:
                        try:
                            ...
                            Logger().get_log().info("request   url：%s" %url_new)
                        ...
                        Logger().get_log().info("请确认第%d行parameters" %row)
                else:  # 参数中不包含&，只有一个参数
        ...
        if data and isinstance(data, str):
            if '&' in str(data):
                ...
                if parameters_data.startswith('{'):
                    ...
                    Logger().get_log().info("请确认第%d行parameters中需要update的值格式，应为id&{data}" %row)
            else:
                ...
                if data.startswith('select id'):  # sql语句的查询结果当做参数
                    ...
                    if data_select_result:
                        try:
                            ...
                            Logger().get_log().info("请确认第%d行SQL语句" %row)
                        else:
                            if len(datas) == 1:
                                Logger().get_log().info("request   data：%s" %datas)
                                ...
                                Logger().get_log().info("请确认 select 语句查询返回值是不是只有一个")
                    else:
                        Logger().get_log().info("第%d行参数查询无结果" %row)
                    # 字典形式作为参数，如{"id":"7135cf6e-2b12-4282-90c4-bed9e2097d57","name":"gbj_for_jdbcDatasource_create_0301_1_0688","creator":"admin"}
        ...
        for row in range(2, all_rows+1):
            ...
            if ex_status_code and ac_status_code != '':
                ...
                if ex_status_code == ac_status_code:
                    ...
                    Logger().get_log().info("预期结果：%s, 实际结果：%s" %(ex_status_code, ac_status_code))
            else:
                Logger().get_log().info("第 %d 行 status_code为空" %row)
        case_table.save(ab_dir('api_cases.xlsx'))
        ...
        for row in range(2, all_rows+1):
            ...
            if code_result == 'pass':
                if key_word in ('create', 'query', 'update', 'delete'):
                    ...
                    Logger().get_log().info("请确认第%d行的key_word" %row)
                ...
                Logger().get_log().info("请确认第 %d 行 status_code对比结果" %row)
        ...
        if key_word == 'create':
            if relation == '=':   # 只返回id时，判断返回内容中包含id属性，id长度为36
                if isinstance(response_text_dict, dict):
                    if response_text_dict.get("id"):
                        ...
                        try:
                            ...
                            Logger().get_log().info("第 %d 行 response_text返回的id和预期id长度不一致" %row)
                        ...
                        try:
                            ...
                            Logger().get_log().info("第 %d 行 response_text和预期text不相等" %row)
                            case_table_sheet.cell(row=row, column=column, value='fail')
                        else:
                            case_table_sheet.cell(row=row, column=column, value='pass')
                ...
                try:
                    ...
                    Logger().get_log().info("第 %d 行 expect_text没有包含在response_text中， 结果对比失败" %row)
                    case_table_sheet.cell(row=row, column=column, value='fail')
                else:
                    case_table_sheet.cell(row=row, column=column, value='pass')
            else:
                Logger().get_log().info("请确认第 %d 行 预期expect_text和response_text的relatrion" %row)
            ...
            if relation == '=':
                ...
                try:
                    ...
                    Logger().get_log().info("第 %d 行 expect_text和response_text不相等， 结果对比失败" %row)
                    case_table_sheet.cell(row=row, column=column, value='fail')
                else:
                    case_table_sheet.cell(row=row, column=column, value='pass')
            else:
            ...
            Logger().get_log().info("请确认第 %d 行 的key_word" %row)
        case_table.save(ab_dir('api_cases.xlsx'))
    # 对比case最终的结果
    def deal_result(self):
        ...
        for row in range(2, all_rows + 1):
            ...
            if status_code_result == 'pass' and response_text_result == 'pass':
                Logger().get_log().info("测试用例-%s pass" % case_table_sheet.cell(row=row, column=2).value)
                ...
                Logger().get_log().info("请确认status code或response.text对比结果")
        case_table.save(ab_dir('api_cases.xlsx'))
