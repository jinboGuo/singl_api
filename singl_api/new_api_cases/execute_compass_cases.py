...
def deal_request_method():
    for i in range(2, all_rows+1):
        ...
        if request_method:
            ...
            if api_name == 'root':  # 用例需要使用root用户登录后操作
                # 根据不同的请求方法，进行分发
...
def post_request_result_check(row, column, url, host, headers, data, table_sheet_name):
    try:
        ...
        if '新增调度任务' in case_detail:
            ...
            Logger().get_log().info("new_data：%s"%new_data)
            ...
            if '&' in str(data):  # 包含多个参数并以&分割
                ...
                if len(parameters) == 1:
                    try:
                        ...
                        return
                    Logger().get_log().info("response.url %s, response.status_code %s,response.text%s " %(response.url, response.status_code,response.text))
                    clean_vaule(table_sheet_name, row, column)
                    write_result(sheet=table_sheet_name, row=row, column=column, value=response.status_code)
                    write_result(sheet=table_sheet_name, row=row, column=column + 4, value=response.text)
        ...
        if data:
            if '&' in str(data):
                ...
                if parameters_data.startswith('{'):
                    ...
                    Logger().get_log().info("请确认第%d行parameters中需要update的值格式，应为id&{data}%s"%row)
            else:
