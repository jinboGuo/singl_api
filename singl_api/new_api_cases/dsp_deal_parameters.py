...
from util.get_deal_parameter import get_resourceid, get_datasource, get_schema, get_tags, get_dataset, get_tenant_id, \
    get_owner
    ...
    try:
        if data:
            ...
            if '数据标准目录' in data:
                ...
                return deal_parameters(data, request_method, request_url)
            if '元数据字段ID' in data:
                data = data.replace('元数据字段ID', str(random.randint(10 ** 17, 10 ** 18 - 1)))
                return deal_parameters(data, request_method, request_url)
            if '&&' in data and 'API数据服务ID' in data:
                ...
                request_data = str(request_data).replace('API数据服务ID', str(data_select_result))
                return deal_parameters(request_data, request_method, request_url)

            if '&&' in data and 'API工单ID' in data:
                select_data = data.split('&&')[0]
                log.info("开始执行语句:{}".format(select_data))
                data_select_result = ms.ExecuQuery(select_data.encode('utf-8'))
                log.info("sql查询结果为:{}".format(data_select_result))
                data_select_result = data_select_result[0]['id']
                request_data = data.split('&&')[1]
                request_data = str(request_data).replace('API工单ID', str(data_select_result))
                return deal_parameters(request_data, request_method, request_url)
            if '&&' in data and '数据源ID' in data:
                select_data = data.split('&&')[0]
                log.info("开始执行语句:{}".format(select_data))
                data_select_result = ms.ExecuQuery(select_data.encode('utf-8'))
                log.info("sql查询结果为:{}".format(data_select_result))
                data_select_result = data_select_result[0]['id']
                request_data = data.split('&&')[1]
                request_data = str(request_data).replace('数据源ID', data_select_result)
                return deal_parameters(request_data, request_method, request_url)
            if '&&' in data and '数据集ID' in data:
                select_data = data.split('&&')[0]
                log.info("开始执行语句:{}".format(select_data))
                data_select_result = ms.ExecuQuery(select_data.encode('utf-8'))
                log.info("sql查询结果为:{}".format(data_select_result))
                data_select_result = data_select_result[0]['id']
                request_data = data.split('&&')[1]
                request_data = str(request_data).replace('数据集ID', data_select_result)
                return deal_parameters(request_data, request_method, request_url)
            if '&&' in data and '["批量数据服务ID"]' in data:
                select_data = data.split('&&')[0]
                log.info("开始执行语句:{}".format(select_data))
                data_select_result = ms.ExecuQuery(select_data.encode('utf-8'))
                log.info("sql查询结果为:{}".format(data_select_result))
                ...
                if len(data_select_result) >= 1:
                    ...
                    request_data = data.split('&&')[1]
                    request_data = str(request_data).replace('["批量数据服务ID"]', str(new_data))
                    # print(request_data)
                    return request_data
                else:
                    print('sql未查询到数据')
            if '&&' in data and '数据服务ID' in data:
                select_data = data.split('&&')[0]
                log.info("开始执行语句:{}".format(select_data))
                data_select_result = ms.ExecuQuery(select_data.encode('utf-8'))
                log.info("sql查询结果为:{}".format(data_select_result))
                data_select_result = data_select_result[0]['id']
                request_data = data.split('&&')[1]
                request_data = str(request_data).replace('数据服务ID', str(data_select_result))
                return deal_parameters(request_data, request_method, request_url)
            if '&&' in data:
                    ...
                    if len(new_data) > 2:
                        ...
                        if '/api/sys/meta/datasets/previewinit?rows=50' == new_data[2]:
                            try:
                                ...
                                response1 = requests.post(url=host + new_data[2], headers=get_headers(),
                                                          data=json.dumps(new_data1))
