...
def get_jobview(data):
    try:
        ...
        new_data = {"name":  "scheduler随机数", "poolName":  s_c_job_pool_info[0]["pool_name"], "poolId":  s_c_job_pool_info[0]["job_pool_oid"], "sliceType":  "H", "enabled":  0, "description":  "自动化脚本", "resourceId":  asset_directory_info[0]["id"]}
        deal_random(new_data)
        return new_data
    except Exception as e:
        log.error("异常信息：%s" % e)
...
def get_jobview_exec(data):
    try:
        ...
        if data =='scheduler&all':
            ...
            return new_data
        elif data =='scheduler&state2':
            new_data = {"fieldGroup":{"fields":[{"andOr":"AND","name":"status","oper":"EQUAL","value":["2"]},{"andOr":"AND","name":"viewId","oper":"EQUAL","value":viewid}]},"ordSort":[{"name":"startTime","order":"DESC"}],"pageable":{"pageNum":1,"pageSize":8,"pageable":True}}
            return new_data
        elif data =='scheduler&state1':
            ...
            return new_data
        elif data =='scheduler&nodename-jobtype2':
...
def publish_flow(data):
    try:
        ...
        job_info = ms.ExecuQuery(sql.encode('utf-8'))
        dataset_sql = " select md.id,md.name,md.schema_id ,ms.name as schema_name from merce_dataset md inner join merce_schema ms on md.schema_id =ms.id where md.name like '%s%%%%' order by md.last_modified_time desc limit 1"% data[1]
