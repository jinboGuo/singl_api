...
def delete_asset_directory(data):
    try:
        data = data.split("&")
        merce_resource_dir = "delete from merce_resource_dir where name like 'gjb%' order by create_time desc"
        ms.ExecuNoQuery(merce_resource_dir.encode('utf-8'))
        merce_resource_dir = "delete from merce_resource_dir where name like 'test_asset%' order by create_time desc"
        ms.ExecuNoQuery(merce_resource_dir.encode('utf-8'))
        merce_dataset = "delete from merce_dataset where name like 'gjb_scheduler_sink%' order by create_time desc"
        ms.ExecuNoQuery(merce_dataset.encode('utf-8'))
        merce_schema = "delete from merce_schema where name like 'gjb_scheduler_sink%' order by create_time desc"
        ms.ExecuNoQuery(merce_schema.encode('utf-8'))
        merce_schema1 = "delete from merce_schema where name like 'auto_apitest%' order by create_time desc"
        ms.ExecuNoQuery(merce_schema1.encode('utf-8'))
        merce_flow = "delete from merce_flow where name like 'BATCH_COLUMN_gjb%' order by create_time desc"
        ms.ExecuNoQuery(merce_flow.encode('utf-8'))
        merce_flow_dataset_map = "delete from merce_flow_dataset_map where dataset_name like 'auto_apitest.default.supp%'"
        ms.ExecuNoQuery(merce_flow_dataset_map.encode('utf-8'))
        merce_flow_dataset_map1 = "delete from merce_flow_dataset_map where dataset_name like 'gjb_scheduler%'"
...
def dc_collecter_group(data):
    try:
        dc_collecter = "delete from dc_collecter where group_name like'%s%%%%' order by create_time desc" % data
