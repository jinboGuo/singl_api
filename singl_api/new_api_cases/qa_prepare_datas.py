...
def create_datasetjob_data(data):
    try:
        new_data = {"jobType": "BATCH_DATASET","name": "api_dana_dataset_job","responsibleId": "1228346912479621120","responsibleName": "admin","description": "api_dana_dataset_job描述","schedulerType": "SINGLE","schedulerCycle": "","schedulerDelay": {    "delay": 1,    "unit": "MINUTE"},"epStorageType": "FTP","iterativeType": "FULL","ruleConf": {    "datasetRuless": [{        "dataset": {            "datasetId": get_dataset('dataset_id', 'dana_api_mysql_dataset'),            "datasetName": "dana_api_mysql_dataset"        },        "rules": [{            "ruleId": get_selfqaruleid('字段联合空值校验', 0),            "ruleName": "字段联合空值校验",            "version": 1,            "weights": 1,            "buildIn": 1,            "epOutFlag": 1,            "parameters": {                "templateType": "T11",                "operator": "and",                "value": ["change", "deal_time"],                "alarmRanges": [0, 25, 50],                "yellowValue": 0,                "orangeValue": 25,                "redValue": 50            }        }, {            "ruleId": get_selfqaruleid('api_dana_dataset_testrule'),            "ruleName": "api_dana_dataset_testrule",            "version": 1,            "weights": 1,            "buildIn": 0,            "epOutFlag": 1,            "parameters": {                "templateType": "T22",                "alarmRanges": [0, 25, 50],                "yellowValue": 0,                "orangeValue": 25,                "redValue": 50,                "refValCycle": 0,                "sqlExpr": "sum( case when #{FIELD0}  in  ('男','女') then 0 else 1 end) ",                "badSqlExpr": " #{SEL_FIELD0}  in  ('男','女')",                "mainTable": "",                "paramPair": {                    "FIELD0": "deal_type",                    "SEL_FIELD0": "deal_type"                }            }        }],        "increment": {            "incrType": "FIELD",            "incrUnit": "DAY",            "incrField": "deal_time",            "incrFormat": "yyyyMMddHH",            "fieldType": "timestamp"        }    }],    "datasetFieldRules": {        "fieldRuless": [],        "conditionSql": "",        "dimensions": [],        "increment": None    }},"notice": {    "noticeTypes": [],    "noticeLevels": [],    "noticeMembers": [],    "noticeMembersName": []},"isExceptionOut": 1,"tempEntity": {    "noticeMembers": [],    "datasets": "dana_api_mysql_dataset"},"exceptionOutConfig": {    "normalOut": False,    "retainCycle": 1,    "epOutPath": "/home/europa/ftp_test",    "datasourceName": "dana_api_ftp_test",    "datasourceId": get_datasource('datasource_id', 'dana_api_ftp_test'),    "epDatasetResourceName": "数据集",    "epDatasetResourceId": get_resourceid('dataset_dir'),    "epSchemaResourceName": "元数据",    "epSchemaResourceId": get_resourceid('schema_dir')},"resourceId": get_resourceid('qa_job_dir')}
        return new_data
    except Exception as e:
        log.error("异常信息：%s" %e)
