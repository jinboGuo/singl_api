...
class CasesForZmod(unittest.TestCase):
    ...
    def test_create_Zdaf_flow(self):
        ...
        response = requests.post(url=create_zmod_flow_url, headers=get_headers(host), json=zmod_id)
    ...
    def test_query_zdaf_all(self):
        ...
        data = {"fieldList": [], "sortObject": {"field": "lastModifiedTime", "orderDirection": "DESC"}, "offset": 0,
                "limit": 8}
        response = requests.post(url=query_zdaf, headers=get_headers(host), json=data)
        # 接口查询返回8条分析任务id并排序
        content_ids = []
    ...
    def test_query_zdaf_by_name(self):
        ...
        response = requests.post(url=self.query_zdaf, headers=get_headers(host), json=data)
        self.assertEqual(200, response.status_code, '分析任务查询接口调用失败')
    ...
    def test_query_zmod_model_detail(self):
        ...
        response = requests.get(url=query_zmod_model_detail_url, headers=get_headers(host))
    ...
    def test_query_zmod_exectuion(self):
        ...
        response = requests.get(url=query_zmod_exectuion_url, headers=get_headers(host))
...
class QueryZmodResult(unittest.TestCase):
    ...
    def test_zdaf_result(self):
        ...
        response = requests.post(url=query_zdaf_result_url, headers=get_headers(host), json=data)
