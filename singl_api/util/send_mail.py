...
def dsp_main3(host,receivers):
    ...
    # 统计api执行结果，加入到邮件正文中，失败的用例name：失败的原因
    api_cases_table = load_workbook(ab_dir('api_cases.xlsx'))
    cases_sheet = api_cases_table.get_sheet_by_name('dsp_199')
    sheet_rows = cases_sheet.max_row
    cases_num = sheet_rows - 1
    pass_cases = 0
    failed_cases = 0
...
def compass_main3(host,receivers):
    # 163邮箱smtp服务器
    host_server = "smtp.163.com"
    # 发件人
    sender_163 = "ruifan_test@163.com"
    ...
    # 统计api执行结果，加入到邮件正文中，失败的用例name：失败的原因
    api_cases_table = load_workbook(ab_dir('api_cases.xlsx'))
    cases_sheet = api_cases_table.get_sheet_by_name('compass')
    sheet_rows = cases_sheet.max_row
    cases_num = sheet_rows - 1
    pass_cases = 0
    failed_cases = 0
    ...
    # print(mail_content)
    # 邮件标题
    mail_title = time.strftime("%Y-%m-%d", time.localtime()) + ' COMPASS系统API用例自动化执行日报'
