...
def get_resourceid(resource_type):
    ...
    try:
        if resource_type == 'schema_name_rule_dir' or resource_type == 'schema_collect_task_dir':
            ...
            return resource_id
        elif resource_type == 'indicator_dir':
            sql = "select id from ind_dir where name = '指标根目录' and ord = '0' and creator  = 'admin' and parent_id  = '0' and tenant_id='%s'" % (
                tenant_id)
            resource_dir = ms.ExecuQuery(sql.encode('utf-8'))
            resource_id = resource_dir[0]["id"]
            return resource_id
        elif resource_type == 'indicator_dim_dir':
            sql = "select id from ind_dim_dir where name = '维度根目录' and ord = '0' and creator  = 'admin' and parent_id  = '0' and tenant_id='%s'" % (
                tenant_id)
            resource_dir = ms.ExecuQuery(sql.encode('utf-8'))
            resource_id = resource_dir[0]["id"]
            return resource_id
...
def get_sink_schema_name():
    tenant_id = get_tenant_id()
    try:
        sql = "select name from merce_schema ms where tenant_id={} and name like 'test_api_wjp_schema%_sink%' ORDER BY create_time desc LIMIT 1".format(
            tenant_id)
        query_data = ms.ExecuQuery(sql.encode('utf-8'))
        schema_name = query_data[0]["name"]
        return schema_name
...
def get_source_dataset_name(num=0):
    """获取采集输入端数据集名称"""
    tenant_id = get_tenant_id()
    try:
        sql = "SELECT name from merce_dataset  where tenant_id= {} and name like 'test_wjp_dataset%' ORDER BY create_time desc limit 2".format(
            tenant_id)
        query_data = ms.ExecuQuery(sql.encode('utf-8'))
        dataset_name = query_data[num]["name"]
        return dataset_name
    except Exception as e:
        log.error("没有获取到输入端数据集名称：%s" % e)


def get_source_dataset_id(num=0):
    """获取采集输入端数据集id"""
    tenant_id = get_tenant_id()
    try:
        sql = "SELECT id from merce_dataset  where tenant_id= {} and name like 'test_wjp_dataset%' ORDER BY create_time desc limit 2".format(
            tenant_id)
        query_data = ms.ExecuQuery(sql.encode('utf-8'))
        dataset_id = query_data[num]["id"]
        return dataset_id
    except Exception as e:
        log.error("没有获取到输入端数据集名称：%s" % e)
...
def get_collect_dir_id():
    """获取采集目录id"""
    tenant_id = get_tenant_id()
    try:
        sql = "select id from merce_resource_dir where tenant_id={} and name like '%s%%%%' GROUP BY create_time desc limit 1".format(
            tenant_id, 'test_api_wjp_collect')
        get_collect_dir_ids = ms.ExecuQuery(sql.encode('utf-8'))
        get_collect_dir_id = get_collect_dir_ids[0]["id"]
        return get_collect_dir_id
    except Exception as e:
        log.error("没有获取到采集目录id：%s" % e)
...
def get_rule_name():
    ...
    try:
        ...
        log.error("没有获取到元数据采集命名规则名称：%s" % e)
...
def indicator_data():
    ...
    try:
        sql_query = """
        SELECT
            current_timestamp() AS biztime,
            t0.uid AS uid,
            round(avg(t0.price), 0) AS test_api_wjp_indicator_sql_8984675949129
        FROM
            {} t0
        LEFT JOIN {} t1 ON t0.sid = t1.sid
        GROUP BY
            t0.uid
        """.format(get_source_dataset_name(1), get_source_dataset_name())
        data = {
            'sql': sql_query
        }
        return data
...
def get_indicator_subtask_id():
    """获取指标子任务id"""
    tenant_id = get_tenant_id()
    try:
        sql = "select id from ind_execution where tenant_id='%s' and name like '%s%%%%' ORDER BY create_time desc limit 1" % (
            tenant_id, 'test_api_wjp_indicator')
        get_indicator_subtask_id = ms.ExecuQuery(sql.encode('utf-8'))
        indicator_subtask_id = get_indicator_subtask_id[0]["id"]
        return indicator_subtask_id
    except Exception as e:
        log.error("没有获取到指标子任务id：%s" % e)


def get_indicator_dir_id():
    """获取指标目录id"""
    tenant_id = get_tenant_id()
    try:
        sql = "select id from ind_dir where tenant_id='%s' and name like '%s%%%%' ORDER BY create_time desc limit 1" % (
            tenant_id, 'test_api_wjp_indicator')
        get_indicator_dir_id = ms.ExecuQuery(sql.encode('utf-8'))
        indicator_dir_id = get_indicator_dir_id[0]["id"]
        return indicator_dir_id
    except Exception as e:
...
def get_indicator_dimdir_id():
    ...
    tenant_id = get_tenant_id()
    try:
        sql = "select id from ind_dim_dir where tenant_id='%s' and name like '%s%%%%' ORDER BY create_time desc limit 1" % (
            tenant_id, 'test_api_wjp_indicator_dim_dir')
        get_indicator_dimdir_id = ms.ExecuQuery(sql.encode('utf-8'))
        indicator_dimdir_id = get_indicator_dimdir_id[0]["id"]
        return indicator_dimdir_id
    except Exception as e:
        log.error("没有获取到指标目录id：%s" % e)


def get_indicator_dim_id():
    """获取指标维度id"""
    tenant_id = get_tenant_id()
    try:
        ...
        indicator_dim_id = get_indicator_dim_id[0]["id"]
        return indicator_dim_id
    except Exception as e:
        log.error("没有获取到指标维度id：%s" % e)


def get_indicator_dim_name():
    """获取指标维度id"""
    tenant_id = get_tenant_id()
    try:

        sql = "select name from ind_dim where tenant_id='%s' and name like '%s%%%%'" % (
            tenant_id, 'test_api_wjp_dim')
        get_indicator_dim_name = ms.ExecuQuery(sql.encode('utf-8'))
        indicator_dim_name = get_indicator_dim_name[0]["name"]
        return indicator_dim_name
    except Exception as e:
        log.error("没有获取到指标维度id：%s" % e)
...
def get_qa_task_id():
    ...
    tenant_id = get_tenant_id()
    try:
        sql = "select id from qa_job where tenant_id='%s' and name like '%s%%%%' ORDER BY create_time desc limit 1" % (
        tenant_id, qa_task_name)
...
def get_dsp_data_resource(dsp_data_source):
    ...
    tenant_id = get_tenant_id()
    try:
        sql = "select id,name from dsp_data_resource where tenant_id='%s' and name like '%s%%%%' ORDER BY create_time desc limit 1" % (
        tenant_id, dsp_data_source_name)
...
def get_dsp_data_application():
    ...
    tenant_id = get_tenant_id()
    try:
        sql = "select id from dsp_data_application where tenant_id='%s' and name like '%s%%%%' ORDER BY create_time desc limit 1" % (
        tenant_id, dsp_data_source_name)
...
def get_sys_approval_target(approval_type):
    ...
    tenant_id = get_tenant_id()
    try:
        sql = "select id from sys_approval_target where tenant_id='%s' and code='%s' and type ='INTERNAL'" % (
